...
def load_data(file_name):
    ...
    label_data = np.mat(np.zeros((m, 1)))
    for i in range(m):
        label_data[i, 0] = label_tmp[i]
...
class RBF_NN():
    ...
    def show_model(self, W, b, c, delta):
        n = 100
        min = 10000.0
        max = -10000.0
        for i in range(self.input_data_trainX.shape[0]):
            if min > self.input_data_trainX[i, 0]:
                min = self.input_data_trainX[i, 0]
            if max < self.input_data_trainX[i, 0]:
                max = self.input_data_trainX[i, 0]
        x = np.linspace(min, max, n)
        y = np.zeros(n)
        for i in range(0, n):
            y[i] = b[0, 0]
            for j in range(0, self.hidden_nodes):
                y[i] = y[i] + W[j, 0] * math.exp(-math.pow(x[i] - c[j, 0], 2) / (2 * math.pow(delta[0, j], 2)))
        plt.plot(x, y)
        plt.show()
